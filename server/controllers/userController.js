...
const registerStudentInfo = async(req, res, next) => {
    ...
    try{
            const { occupation, institute_name, stream, year, domain, state, country } = req.body
            ...
            let result = await StudentInfo.create({
                user_info_id, institute_name, stream, year, domain, state, country 
            }, {transaction})
        ...
        return res.status(500).json(resData(false, 'user_info already exits', null))
    }
}
...
const registerCompanyInfo = async(req, res, next) => {
    ...
    try{
            const { occupation, company_name, company_size, sector, website, state, country } = req.body
            ...
            let result = await CompanyInfo.create({
                user_info_id, company_name, company_size, sector, website, state, country 
            }, {transaction})
...
const updatedUserInfo = asyncWrapper( async(req, res, next) => {
    ...
    if(!data) return res.status(400).json(resData(false, 'user data not found', null))
    ...
    let updatedUserData = await table.update(
        req.body,
        {where : { user_info_id: data.id }, returning: true }
    )
